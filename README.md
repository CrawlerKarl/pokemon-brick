...
| `run-suite.js` | **The release gate (`npm test`)**: syntax → assets → the 127-check suite headless → both-skin + dist boot smokes → vocabulary scan → 50 mobile scenes that each PROVE their named state (`expect` + metadata sidecars) under the AFT-021 overlap and single-actor-label contracts → wave and boss artifact-storm benchmarks. ~30–35s, zero deps (raw CDP over Node's WebSocket + system Chrome) |
| `run-baseline.js` | **The campaign matrix (`npm run baseline`)**: 162 deterministic autopilot scenarios (~95-105s) — all 27 stages, all nine finales × three modes × three seeds, **three full campaign runs**, paths/fusions/**apexes (3 seeds each)**, **vessels (3 seeds each)**, difficulties (+ drift-policy variants), affinities, the AEGIS economy — with the duration/spread/recovery/**apex-identity/draft-cadence** BUDGETS enforced (red exit on violation). `--label` names each report's provenance (`aft021`, `redesigned-campaign`, …); `BASE_ONLY=<substring>` filters scenarios for tuning loops (always pair it with `--label` or you'll overwrite the committed old-campaign fixtures). Committed snapshots in `docs/baselines/` |
...
**`npm test` is the release gate.** It runs syntax + asset checks, 127 headless
invariants, both-edition boot smokes, the standalone build and vocabulary
scan, 50 mobile reference scenes (25 scenes × 2 viewports, each proving its
